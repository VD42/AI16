...
void MyStrategy::move(const model::Wizard & self, const model::World & world, const model::Game & game, model::Move & move)
{
	...
	if (m_bCatchBonuses)
	{
		...
		{
			if (m_self->getDistanceTo(1200.0, 1200.0) <= 1800.0 && (m_global.CanGoToBonusT() || m_self->getDistanceTo(1200.0, 1200.0) <= 400.0))
		...
		{
			if (m_self->getDistanceTo(2800.0, 2800.0) <= 1800.0 && (m_global.CanGoToBonusB() || m_self->getDistanceTo(2800.0, 2800.0) <= 400.0))
...
void MyStrategy::BestShoot(const model::CircularUnit & unit, bool turn)
{
	...
	int nCreepsCount = 0;
	for (auto & creep : m_world->getMinions())
	{
		if (creep.getId() == unit.getId())
			continue;
		if (creep.getFaction() == m_self->getFaction())
			continue;
		if (creep.getDistanceTo(unit) < unit.getRadius() - creep.getRadius() + m_game->getFireballExplosionMinDamageRange())
			nCreepsCount += 1;
	}
	for (auto & wizard : m_world->getWizards())
	{
